...
contract Casino {
    ...
    function getMinimumBets() public view returns(uint8){
        return minimumBets;
    }

    function getWinnersPercentage() public view returns(uint8){
    ...
    function endBets() external isAdmin {
        ...
        uint8 winnersCount = (uint8(participants.length) * winnersPercentage) / 100;
    ...
    function setMinimumBets(uint8 _count) public isAdmin {
        minimumBets = _count;
    }

    function setWinnersPercentage(uint8 _percent) public isAdmin {
        winnersPercentage = _percent;
    }
}
